...
public class OrderServiceImpl implements OrderService {
		            ...
		            if (currentStock >= odr_amount) {
		                ...
		                if (updateSuccess) {
		                    ...
		                    if (insertSuccess==1) {
		    					...
		    					//메시지 저장
		    					omDao.insert(new OrderMsgDto(0, pdto.getProduct_code(), pdto.getProduct_name() + "상품 주문 완료."));
		                        return true;
	...
	public boolean removeOrder(String sid,String order_id)
	{
		...
		try {
			result += omDao.delete(order_id);//ordermessage 삭제 정상 실행 확인 코드
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(result > 0)
